...
class ScrabbleGame {
    ...
    private fun initPremiumSquares() {
        listOf(0 to 0, 0 to 7, 0 to 14, 7 to 0, 7 to 14, 14 to 0, 14 to 7, 14 to 14).forEach { premiumSquares["${it.first},${it.second}"] = "TW" }
        listOf(1 to 1, 2 to 2, 3 to 3, 4 to 4, 10 to 10, 11 to 11, 12 to 12, 13 to 13, 1 to 13, 2 to 12, 3 to 11, 4 to 10, 10 to 4, 11 to 3, 12 to 2, 13 to 1).forEach { premiumSquares["${it.first},${it.second}"] = "DW" }
        listOf(1 to 5, 1 to 9, 5 to 1, 5 to 5, 5 to 9, 5 to 13, 9 to 1, 9 to 5, 9 to 9, 9 to 13, 13 to 5, 13 to 9).forEach { premiumSquares["${it.first},${it.second}"] = "TL" }
        listOf(0 to 3, 0 to 11, 2 to 6, 2 to 8, 3 to 0, 3 to 7, 3 to 14, 6 to 2, 6 to 6, 6 to 8, 6 to 12, 7 to 3, 7 to 11, 8 to 2, 8 to 6, 8 to 8, 8 to 12, 11 to 0, 11 to 7, 11 to 14, 12 to 6, 12 to 8, 14 to 3, 14 to 11).forEach { premiumSquares["${it.first},${it.second}"] = "DL" }
    }
    ...
    fun startGame(player1Name: String, player2Name: String) {
        ...
        repeat(7) { if (bag.isNotEmpty()) p1Rack.add(bag.removeAt(bag.size - 1)); if (bag.isNotEmpty()) p2Rack.add(bag.removeAt(bag.size - 1)) }
    ...
    private fun isContiguous(isHorizontal: Boolean): Boolean {
        ...
        for (i in 0 until sorted.size - 1) {
            val cur = if (isHorizontal) sorted[i].col else sorted[i].row; val next = if (isHorizontal) sorted[i + 1].col else sorted[i + 1].row
        ...
        while (cr in 0..14 && cc in 0..14) {
            ...
            if (isNew) {
                when (premiumSquares["$cr,$cc"]) {
                    "DL" -> letterValue *= 2; "TL" -> letterValue *= 3; "DW" -> wordMultiplier *= 2; "TW" -> wordMultiplier *= 3
                }
    ...
    private fun endGame() { players.forEachIndexed { i, p -> val penalty = p.rack.sumOf { letterValues[it] ?: 0 }; players[i] = p.copy(score = maxOf(0, p.score - penalty)) }; isGameOver = true }
    fun reset() { board.forEach { row -> for (i in row.indices) row[i] = null }; placedThisTurn.clear(); players.clear(); currentPlayer = 0; consecutiveSkips = 0; isGameOver = false; bag.clear() }
}
